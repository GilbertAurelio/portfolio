...
(SELECT description FROM adempiere.ad_org organ WHERE organ.ad_org_id = $P{AD_Org_ID} LIMIT 1) AS nama_pt
...
$P{BatchID} LIMIT 1) IS NOT NULL
    ...
    WHERE
    branchtype LIKE (CASE WHEN $P{BranchType} IS NOT NULL THEN $P{BranchType} ELSE '%' END)
    AND categoryname LIKE (CASE WHEN $P{CategoryName} IS NOT NULL THEN $P{CategoryName} ELSE '%' END)